...
fig_dir = join(proj_dir, 'Concepts','figures',concept_name)
os.makedirs(fig_dir, exist_ok=True)

# +

# load location of the concept to plot
...
grid_p = grid_r.merge(proj_zon_stats)
grid_r = grid_r.rename(columns={'node':'node_p', 'row':'row_p','column':'column_p'})


rot = 52.9

# currently the project sites have the same row,columns but in case of refinement
# the grid_match should be kept
...
# +
# the lak_exent is to define the sub area within the concept grid cells that is the primary floodplain
# lak_extent = gpd.read_file(join(main_concept_dir, )
...
def main_map(ax, lak_extent=None):
    ## main figure ##
...
arr_lab(m_domain, 'Concept\nModel Extent', ax, offset = (-600, -0), fontsize=fontsize)

# ax.legend(handles=lgd_short, loc='upper left')

dir_arrow(ax, 0.8, 0.7, -0.075, -0.075, -0.05, 'Streamflow\nDirection', fontsize=8)
...
fig.savefig(join(fig_dir, 'domain_map_with_CA_inset.png'), bbox_inches='tight')
...
m_domain.plot(color="none", edgecolor='black',ax=ax)
gdf_bnds(m_domain, ax=ax, buf=1E3)
...
loadpth = 'C:/WRDAPP/GWFlowModel/Cosumnes/Regional/EcoFIP/'


model_ws = join(loadpth, concept_name)
model_ws
# -
...
def ij_to_df(i,j):
    ''' translate array of i,j (0-based) to dataframe with 1-based'''
    df = pd.DataFrame(np.transpose([i, j]), columns=['i','j'])
    df[['row','column']] = df[['i','j']]+1
    return df


# where there are GDEs does not necessarily align with the local concepts
gde_row, gde_col = np.where(m.evt.exdp[0].array>2)
gde_df = ij_to_df(gde_row, gde_col)
gde_grid = grid_p.merge(gde_df)
...
wel_df = ij_to_df(wel_row, wel_col)
...
grid_sfr = reach_data_gdf(m.sfr, grid_r.rename(columns={'row_p':'row','column_p':'column'}))
...
riv_arr = spd_2_arr(m.riv.stress_period_data, sp_col = 'stage', dis=m.dis)
riv_row, riv_col = np.where(riv_arr.mean(axis=0)>0)
riv_df = ij_to_df(riv_row, riv_col)
# keep name as lake for consistency
lak_grid = grid_p.merge(riv_df)
lak_grid_union = gpd.GeoDataFrame([0], geometry = [lak_grid.geometry.unary_union], crs=grid_p.crs)
...
legend_elements = [
    ...
    Line2D([0], [0],color='None', marker='.', markerfacecolor='black', label='Pumping Wells', linewidth=4),
...
def make_river(grid_sfr, lak_grid, tprogs_lay):
    ...
    r_lay = tprogs_lay[grid_sfr.i.astype(int), grid_sfr.j.astype(int)]
    river_arr[r_lay-4, grid_sfr.i.astype(int), grid_sfr.j.astype(int)] = 1
    
    lak_lay = tprogs_lay[lak_grid.i.astype(int), lak_grid.j.astype(int)]
    river_arr[lak_lay-2, lak_grid.i.astype(int), lak_grid.j.astype(int)] = 1
...
# for t in [0,1,2]:
    ...
    grid_plt(tprogs_local_active, join(tprogs_figs,'tprogs_local_facies_r'+str(r).zfill(3)), river, z_scale = 20, y_rot = 10)
    # grid_plt(tprogs_active, join(tprogs_figs,'tprogs_facies_r'+str(r).zfill(3)), make_river(grid_sfr_p, lak_grid, tprogs_lay))
