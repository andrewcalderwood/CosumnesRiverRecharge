...
C-------CALCULATE THE MAXIMUM DEPTH
      ...
      WpN = Wetperm/Daq
      dpN = maxdepth/Daq
      Aniso = VKA(JRCH, IRCH, 1)/HK(JRCH,IRCH,1)
      rho = SQRT(Aniso)
      stdfar = 2*Daq/rho
      deltastd = stdfar-2*Daq
C     MODFLOW GRID CELL SIZE MAY CHANGE IF DELR!=DELC
      delta = ((DELR(JRCH)/4)-(maxwidth/2))-stdfar
      xi = (1-SQRT(dpN))*(1-rho)
...
C     DIMENSIONLESS CONDUCTANCE ACCOUNTING FOR DEGREE OF PENETRATION
      ELSE 
        gflatrect = gflat*(1+aone*dpN+atwo*dpN*dpN) 
      END IF
...
C     DIMENSIONLESS CONDUCTANCE WITH CORRECTION FOR A CLOGGIN LAYER
      ganisostddeltarcl = ganisostddelta/(1+ganisostddelta
     +                   *(HK(JRCH,IRCH,1)/avhc)*(sbdthk/Wetperm))
C     CALCULATE THE ACTUAL STREAMBED CONDUCTANCE
C     Two times the one side conductance
      Cstr  = 2*HK(JRCH,IRCH,1)*strlen*ganisostddeltarcl 
C
CS-----RETURN.
      RETURN
