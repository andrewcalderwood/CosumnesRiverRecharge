...
C Need Daq, B, H, Kv, Kh, Kcl, ecl, D, Wp
                CALL GWF2SFR7SAFE(istsg, wetperm, depth, rchlen 
...
C         EQUAL TO 2.
              ...
              IF ( h.GT.sbot ) THEN
                ...
                IF( icalc.EQ.5) THEN
                  CALL GWF2SFR7SAFE(istsg, wetperm2, strlen, avthk,
     +                              avhc, IRCH, JRCH, cstr)
...
Cdep revised pp1,pp2,et1,and et2, wrong placement of parenthesis.
     ...
     +                              avthk, avhc, IRCH, JRCH, cstr1)
                  CALL GWF2SFR7SAFE(Istsg, wetperm2+dlwp2*dlh, strlen,
     +                              avthk, avhc, IRCH, JRCH, cstr2)
                END IF
C
...
C49-----USE BISECTION WHEN LEAKAGE IS LIMITED BY FLOW IN CHANNEL.
     ...
     +                          FOLDFLBT(lfold))
                  ELSE
                    flobotp = cstr1*((deptfhp+strtop)-sbot)
                  END IF
...
C64-----DEFINE HSTR, CSTR, WIDTH, AND FLOWOT.
            ...
            IF ( icalc.EQ.5 ) THEN
              CALL GWF2SFR7SAFE(Istsg, wetperm, strlen, avthk, 
     +                          avhc, IRCH, JRCH, cstr)
            END IF         
          END IF
C
              ...
              IF ( flowc.LT.NEARZERO ) THEN
                ...
                ELSE
                  IF (icalc.EQ.5 .AND. h.GT.sbot) THEN
                    CALL GWF2SFR7SAFE(Istsg, width, strlen, avthk, 
     +                                avhc, IRCH, JRCH, cstr) 
                  END IF
                  flobot = cstr*(hstr-h)
                  IF ( runof.LT.NEARZERO ) THEN
...
C     Two times the one side conductance
      Cstr  = 2*HK*strlen*ganisostddeltarcl 
C
CS-----RETURN.
      RETURN
      END
