...
C15-----COMPUTE UNSATURATED VARIABLE WHEN SPECIFIED BY SEGMENT.
      IF ( IUZT.EQ.1 ) THEN
        ...
        DO nseg = 1, NSS
          ...
          seglen = SEG(1, nseg)
          sumlen = 0.0
          IF ( icalc.EQ.1 .OR. icalc.EQ.2 .OR. icalc.EQ.5 ) THEN
...
!dep 4/28/2008 end of change
     ...
     +             = (avhc*STRM(5, irch)*rchlen)/avthk
            ELSE IF ( icalc.EQ.1 ) THEN
              ...
              IF ( ksfropt.EQ.1 ) STRM(16, irch)
     +             = (avhc*STRM(5, irch)*rchlen)/avthk
            ELSE IF ( icalc.GE.2 .AND. icalc.LE.5 ) THEN
              STRM(5, irch) = 1.0
              STRM(7, irch) = 1.0
...
C22-----CHECK VALUES IN STREAM CROSS SECTION LIST (XSEC).
        DO nseg = 1, NSS
          icalc = ISEG(1, nseg)
          IF ( icalc.EQ.2 .OR. icalc.EQ.5 ) THEN
        ...
        DO nseg = 1, NSS
          ...
          IF ( icalc.EQ.1 ) THEN
            ...
            IF ( rough.LE.0.0 ) THEN
              ...
              CALL USTOP(' ')
            END IF
          ELSE IF ( icalc.EQ.2 .OR. icalc.EQ.5 ) THEN
            roughch = SEG(16, nseg)
...
C30-----DETERMINE VARIABLES WHEN UNSATURATED FLOW IS ACTIVE.
      DO l = 1, NSTRM
        ...
        istsg = ISTRM(4, l)
        icalc = ISEG(1, istsg)
        IF ( icalc.EQ.2 .AND. IUZT.EQ.1 .OR.
     +    icalc.EQ.5 .AND. IUZT.EQ.1) CALL CHANNELAREA(istsg, l)
...
! RGN 5/9/09 set slope for all icalc
          ...
          IF ( icalc.EQ.1 .OR. icalc.EQ.2 .OR. icalc.EQ.5 ) THEN
            ...
            IF(icalc.EQ.2 .OR. icalc.EQ.5) roughbnk = SEG(17, istsg)
          END IF
...
C         CROSS-SECTIONAL AREA.
                  ELSE IF ( dlkstr.GT.NEARZERO .AND. icalc.EQ.2 .OR.
     +              icalc.EQ.5 ) THEN
...
C25-----INITIALIZE UNSATURATED ZONE VARIABLES.
          ...
          areamax = 0.0
          IF ( IUZT.EQ.1 ) THEN
            IF ( icalc.EQ.1 .OR. icalc.EQ.2 .OR. 
     +         icalc.EQ.5 ) icalccheck = 1
...
C29-----CALCULATE AN INITIAL ESTIMATE OF FLOW IN CHANNEL.
     ...
     +                           istsg)
              ...
              IF ( icalc.EQ.5) THEN
C Need Daq, B, H, Kv, Kh, Kcl, ecl, D, Wp
                CALL GWF2SFR7SAFE(istsg, wetperm, strlen, 
     +                             sbdthk, avhc, ir, ic, cstr)
              END IF
...
C         EQUAL TO 2.
              ...
              IF ( h.GT.sbot ) THEN
                ...
                IF( icalc.EQ.5) THEN
                  CALL GWF2SFR7SAFE(istsg, wetperm2, strlen, sbdthk,
     +                              avhc, ir, ic, cstr)
...
Cdep revised pp1,pp2,et1,and et2, wrong placement of parenthesis.
                ...
                IF (icalc.EQ.5) THEN
                  CALL GWF2SFR7SAFE(Istsg, wetperm1+dlwp1*dlh, strlen,
     +                              sbdthk, avhc, ir, ic, cstr1)
                  CALL GWF2SFR7SAFE(Istsg, wetperm2+dlwp2*dlh, strlen,
     +                              sbdthk, avhc, ir, ic, cstr2)
                END IF
C
...
C49-----USE BISECTION WHEN LEAKAGE IS LIMITED BY FLOW IN CHANNEL.
     ...
     +                          nstrpts, nreach, istsg, Kkiter, 0)
                  ...
                  IF ( h.GT.sbot ) THEN
                    IF ( icalc.EQ.5 ) THEN
                    CALL GWF2SFR7SAFE(Istsg, wetpermp, strlen, sbdthk,
     +                                avhc, ir, ic, cstr1)
                    END IF
...
C64-----DEFINE HSTR, CSTR, WIDTH, AND FLOWOT.
            ...
            IF ( icalc.EQ.5 ) THEN
              CALL GWF2SFR7SAFE(Istsg, wetperm, strlen, sbdthk, 
     +                          avhc, ir, ic, cstr)
            END IF         
          END IF
C
...
crgn added next 4 lines.
              ...
              IF ( flowc.LT.NEARZERO ) THEN
                ...
                IF ( h.LT.hstr ) THEN
                  flobot = 0.0D0
                ELSE
                  IF (icalc.EQ.5 .AND. h.GT.sbot) THEN
                    CALL GWF2SFR7SAFE(Istsg, width, strlen, sbdthk, 
     +                                avhc, ir, ic, cstr) 
                  END IF
                  flobot = cstr*(hstr-h)
                  IF ( runof.LT.NEARZERO ) THEN
...
C4------PRINT CROSS-SECTIONAL DATA FOR SEGMENTS WITH ICALC=2.
      ...
      DO nseg = Lstbeg, lstend
        ...
        END IF
        icalc = ISEG(1, nseg)
        IF ( icalc.EQ.2 .AND. iflg.EQ.0 .OR.
     +    icalc.EQ.5 .AND. iflg.EQ.0) THEN
          WRITE (IOUT, 9025)
 9025     FORMAT (1X, /1X, ' EIGHT POINT CROSS-SECTION DATA ', 
     ...
     +            '        X5        X6        X7        X8')
          iflg = 1
        END IF
        IF ( icalc.EQ.2 .AND. iflg.EQ.1 .OR. 
     +     icalc.EQ.5 .AND. iflg.EQ.1) THEN
...
C-------SUBROUTINE GWF2SFR7SAFE
      SUBROUTINE GWF2SFR7SAFE(Istsg, Wetperm, strlen, 
     +                        sbdthk, avhc, IRCH, JRCH, Cstr)
C     ******************************************************************
...
C     ******************************************************************
      ...
      USE GLOBAL,       ONLY: BOTM, DELR
C      +                        HNEW, DELC, NCOL, NROW, NLAY
      USE GWFLPFMODULE, ONLY: VKA, HK, LAYVKA,  
C       USE GWFHUFMODULE, ONLY: HGUVANI, NHUF, HKHUF=>HK, VKAH
      IMPLICIT NONE
...
C     ------------------------------------------------------------------
      ...
      REAL avhc, strlen
      DOUBLE PRECISION Wetperm, Cstr, sbdthk
C     ------------------------------------------------------------------
C     LOCAL VARIABLES
C     ------------------------------------------------------------------
      DOUBLE PRECISION WpN, dpN, rho, Daq, maxdepth, maxwidth,
     +                 stdfar, deltastd, delta, kappa, PI, gflat, 
     +                 gflatrect, grectisostd, ganisostd, 
     +                 ganisostddelta, ganisostddeltarcl, xi, Rfxi,
     +                 HK, VKA, LAYVKA, XSEC, BOTM, DELR
      REAL aone, atwo, Aniso
C     ------------------------------------------------------------------
C
...
C       Daq = BOTM(JRCH, IRCH, 0 ) - BOTM(JRCH, IRCH, 1)
C C       Daq = 25.0
C C-------CALCULATE THE MAXIMUM DEPTH
C       IF( XSEC(9, Istsg) .LE. XSEC(16, Istsg)) THEN
C         maxdepth = XSEC(9, Istsg)
C       ELSE IF ( XSEC(9, Istsg) .GT. XSEC(16, Istsg) ) THEN
C         maxdepth = XSEC(16, Istsg)
C       END IF
C       maxwidth = XSEC(8, Istsg)
C       WpN = Wetperm/Daq
C       dpN = maxdepth/Daq
C       Aniso = HK(JRCH,IRCH,1)/VKA(JRCH, IRCH, 1)
C       Aniso = LAYVKA
C C       Aniso = 10.0
C       rho = 1/SQRT(Aniso)
C       stdfar = 2*Daq/rho
C       deltastd = stdfar-2*Daq
C C     MODFLOW GRID CELL SIZE MAY CHANGE IF DELR!=DELC
C C     DELR(JRCH)/4
C       delta = (DELR(JRCH)/4)-(maxwidth/2))-stdfar
C       xi = (1-SQRT(dpN))*(1-rho)
C       Rfxi = 1-(0.333*xi)-(0.294*xi**2)
C C
C CS2-----CURVE FIT TABLE INFORMATION
C       IF (WpN.LT.1 .AND. dpN.LT.0.2) THEN
C         aone = 0.89
C         atwo = -2.43
C       ELSE IF (WpN.LT.1 .AND. dpN.LT.0.5) THEN
C         aone = 0.538
C         atwo = -0.387
C       ELSE IF (WpN.LT.3 .AND. dpN.LT.0.2) THEN
C         aone = 0.819
C         atwo = -1.34
C       ELSE IF (WpN.LT.3 .AND. dpN.LT.0.5) THEN
C         aone = 0.672
C         atwo = -0.542
C       ELSE IF (WpN.LT.3 .AND. dpN.LT.0.9) THEN
C         aone = 0.567
C         atwo = -0.33
C       END IF
C CS3-----DIMENSIONLESS CONDUCTANCE CALCULATIONS
C C       PI=4.D0*DATAN(1.D0)
C       PI = 3.141592653589793
C C     DIMENSIONLESS CONDUCTANCE ASSUMING FLAT RECHARGE AREA WITH NO
C C     PENETRATION
C C     If WpN is too small then gflat has a divide by zero
C C       IF (WpN.LT.0.001) WpN = 0.001
C       kappa = EXP(-1*PI*WpN/2)
C       gflat =1/(2*(1+1/(PI*LOG(2/(1-kappa)))))
C       IF( WpN.GE.3) THEN
C         gflatrect = gflat
C C     DIMENSIONLESS CONDUCTANCE ACCOUNTING FOR DEGREE OF PENETRATION
C       ELSE 
C         gflatrect = gflat*(1+aone*dpN+atwo*dpN**2) 
C       END IF
C C     DIMENSIONLESS CONDUCTANCE WITH CORRECTION FOR STANDARD FAR
C C     DISTANCE FROM THE RIVER BANK
C       grectisostd = gflatrect/(1+gflatrect*deltastd/Daq)
C C     DIMENSIONLESS CONDUCTANCE WITH CORRECTION FOR ANISOTROPY 
C       ganisostd = grectisostd*Rfxi
C C     DIMENSIONLESS CONDUCTANCE WITH CORRECTION FOR A GRID DISTANCE
C C     EXCEEDING THE STANDARD FAR DISTANCE 
C       ganisostddelta = ganisostd/(1+ganisostd*delta/Daq)
C C     DIMENSIONLESS CONDUCTANCE WITH CORRECTION FOR A CLOGGIN LAYER
C C       HK(JRCH,IRCH,1), 0.00864
C       ganisostddeltarcl = ganisostddelta/(1+ganisostddelta
C      +                   *(HK(JRCH,IRCH,1)/avhc)*(sbdthk/Wetperm))
C C     CALCULATE THE ACTUAL STREAMBED CONDUCTANCE
C C     Two times the one side conductance
C       Cstr  = 2*HK(JRCH,IRCH,1)*strlen*ganisostddeltarcl 
C
CS-----RETURN.
      RETURN
