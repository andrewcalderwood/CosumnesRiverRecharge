...
# - final choice: average every 4 segments
...
for n in np.arange(0,xs_all_df.shape[1])[:10]:
...
print('Avg RMSE <1 is %.i segments' %np.median(np.sum(xs_mse<1, axis=1)))
...
# %%
XSg_z['easting'] = XSg_z.geometry.x
XSg_z['northing'] = XSg_z.geometry.y
