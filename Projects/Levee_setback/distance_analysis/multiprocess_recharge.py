...
soil_thick=4
# soil_name = '' # default is no extra name
soil_name = '_new' # version from Maples
# fn = chan_dir+'/tprogs_geomK_'+str(soil_thick)+'m_depth.tsv' # old version with linear dem and old function for sampling tprogs
fn = chan_dir+'/tprogs_geomK_'+str(soil_thick)+'m_depth_dem_mean'+soil_name+'.tsv'
# units of m/day
...
def run_rech(t):
    region = 'regional'+soil_name
    for ft in [1,2,3]:
        # 1, 2, 3 are floods long enough to apply to analysis
        realization_recharge(t, str_setbacks, region, ft)
