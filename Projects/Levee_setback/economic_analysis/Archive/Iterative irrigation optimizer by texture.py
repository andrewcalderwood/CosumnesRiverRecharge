...
gen_dict = {**var_gen.to_dict(), **var_crops.to_dict()}
...
# %%
def my_round(n, order):
    return np.round(n/order,0)*order
my_round(13.3, 0.5)


# %%
def log_round(n, order = 1):
    e = np.log10(n).round()
...
# %% [markdown]
# upon first glance it seems that grouping by soil texture is ideal because the crop water requirements are much more similar. The timing for the solution is on the order of 0.05-0.1 from 0.3-0.5, but after converting it to a loop format then it runs slow again.

# %%
textures = soil_crop.groupby('Texture').count()['UniqueID']
...
irr_all =np.zeros((0, 2*n_irr))
p_all = np.ones(0) # make initial profit negative
t_all = np.zeros(0)
...
# identify unique textures as they will have more similar crop water requirements
soil_crop.loc[soil_crop.Texture.isna(), 'Texture'] = 'other'
textures = soil_crop.groupby('Texture').count()
for nt, texture in enumerate(textures.index):
    # grouping by soil texture helps but is dominated by one texture 
    t_bool = (soil_crop.Texture == texture).values
    soil_texture = soil_crop[soil_crop.Texture == texture]
    nfield_texture = soil_texture.shape[0]
    
# for ns in np.arange(0,nfield_crop):
#     soil_ag = soil_crop.iloc[[ns]] #keep as dataframe for consistency

    irr_t =np.zeros((nfield_texture, 2*n_irr))
    p_t = np.ones(nfield_texture) # make initial profit negative
    t_t = np.zeros(nfield_texture)

    for ns in np.arange(0,nfield_texture):
        ns
        soil_ag = soil_texture.iloc[[ns]] #keep as dataframe for consistency 
        
        nfield = soil_ag.shape[0]
        # dtw_arr = dtw_all[:,ns]
        dtw_arr = crop_dtw[:,ns]
        dtw_arr = crop_dtw[:, t_bool][:,ns]
        ...
        while p_t[ns] >0 :
            ...
            irr_t[ns] = out.x
            p_t[ns] = out.fun
            t_t[ns] = out.execution_time
        # print final results
        print('Soil ', str(ns),'%.2f' %(-out.fun),'$ ,in %.2f' %(out.execution_time/60),'min')
    # concatenate
    irr_all = np.append(irr_all, irr_t, axis=0)
    p_all = np.append(p_all, p_t, axis=0)
    t_all = np.appendt_all, t_t, axis=0)

t1 = time.time()
print('Total time was %.2f min' %((t1-t0)/60))

# %% [markdown]
# It only shaved 2 min off 103 to group by texture so it is not a significant savings. It is likely better to find a way to simply group and run in parallel but there must be some way to improve the optimization
