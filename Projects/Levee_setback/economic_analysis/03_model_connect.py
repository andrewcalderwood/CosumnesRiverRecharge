...
# # Define groundwater elevation sampling
    ...
    # %%
    model_ws_last = join(loadpth, 'crop_modflow/'+str(all_run_dates.loc[m_per-1].date.date()))
...
# - this submodel may need to be in a separate script to run multiprocessing (parallel) as this re-loads the entire active script each time.
    ...
    crop_in.to_csv(join(model_ws, 'field_SWB', 'crop_parcels_'+str(year)+'.csv'))
...
# # Run update modflow
    ...
    if m_per > 0:
        mf_ws_last = join(loadpth, 'crop_modflow/'+str(all_run_dates.loc[m_per-1].date.date()))
